  ...
  .handler(async ({ data: request, context }): Promise<MatchActionResult> => {
    ...
    switch (data.type) {
      ...
      case "anticipate":
        ...
        // est en main et qu'aucun pli n'est engagé (`canAnticipate`) : un
        // client modifié ne peut donc pas arrêter le tour au milieu d'un pli
        // pour faire disparaître la carte que l'adversaire vient d'engager.
